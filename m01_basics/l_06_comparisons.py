...
for device in devices:
    print(
        f'{device["name"]:>7} {device["vendor"]:>18} : {device["os"]:<6}   {device["ip"]:<15} {device["version"][:-4]}'
    )
...
print("\n----- Create list of non-compliant devices os versions for each vendor:os ------")
...
if devices2 == devices:
    ...
    print("\n     Deep cpy and modification: devices2 no longer equals devices")
    print("     --> Moral: 'deepcopy()' gives you a complete copy of the original")
    print("     --> Result: I can do whatever I want wth my copy, without touching the original!!")
...
for device in devices:
    ...
    if device["response_time"] > SLA_RESPONSE_TIME:
        print(
            f"{datetime.now()}: {device['name']:6}  - Response Time {device['response_time']:.3} > {SLA_RESPONSE_TIME}")
...
d1_different = DeviceWithEq("device2", "10.10.10.2")

print("\n   Comparison with an __eq__ method to handle evaluation")
if d1 == d1_equal:
    print(f"    ---> using__eq__ : success: {d1} equals {d1_equal}")
else:
    print(f"     !!! using __eq__ : oh-oh, classes not equal and they should be")
if d1 == d1_different:
