...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ip_to_find in devices_dict:
            ...
            print(f"--- ---> id of devices:", id(devices_dict[ip_to_find]))

            print(f"conclusion: dictionary search was {int(generator_search_time/dict_search_time)}"
                  + "time faster than generator ")

    # Simple Generator Comprehension
    print("\n\n____ Device info parsing using generator comprehension")
    device_str = "  r3-L-n7, cisco, catalyst 2960, ios , extra stupid stuff "
    devices_gen = (item for item in device_str.split(","))
    device = [item.strip() for item in devices_gen]
    print("\ndevice using generator comprehension:\n\t\t", device)

    # More interesting Generator Comprehension
    print("\n\n____ Device Creation using generator comprehension")
